...
function parseJsonInput(raw, { expectObject = false, ejson } = {}) {
  ...
  if (expectObject) {
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      throw new Error('JSON input must describe an object.');
    }
  }
...
async function main() {
  const [uri, collectionName, filterJson, limitValue] = process.argv.slice(2);

  if (!uri || !collectionName || !filterJson) {
    console.error('Missing MongoDB connection URI, collection name, or filter JSON.');
    process.exit(2);
    return;
  }
  ...
  let filter;
  try {
    filter = parseJsonInput(filterJson, { expectObject: true, ejson });
  } catch (error) {
    console.error(error.message || 'Failed to parse filter JSON.');
    process.exit(3);
    return;
  }
  ...
  try {
    ...
    const db = client.db();
    const cursor = db.collection(collectionName).find(filter).limit(limit);
